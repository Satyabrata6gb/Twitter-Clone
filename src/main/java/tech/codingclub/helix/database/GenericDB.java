...
public class GenericDB<T> {
    ...
    public static List<?> getRows(TableLike<?> table, Class<?> converterClass, Condition condition, Integer limit, SortField<?> orderBy) {
        ...
        try {
            ...
            SelectJoinStep<Record> joinQuery = create.select().from(table);
            SelectConditionStep<Record> selectCondition = null;
            SelectOffsetStep<Record> y;

            if (condition != null) {
                selectCondition = joinQuery.where(condition);
            }

            if (limit == null) {
                if (selectCondition == null) {
                    if (orderBy != null) {
                        return joinQuery.orderBy(orderBy).fetch().into(converterClass);
                    }
                    return joinQuery.fetch().into(converterClass);
                } else {
                    if (orderBy != null) {
                        return selectCondition.orderBy(orderBy).fetch().into(converterClass);
                    }
                    return selectCondition.fetch().into(converterClass);
                }
            } else {
                if (selectCondition == null) {
                    if (orderBy != null) {
                        y = joinQuery.orderBy(orderBy).limit(limit);
                    } else {
                        y = joinQuery.limit(limit);
                    }
                } else {
                    if (orderBy != null) {
                        y = selectCondition.orderBy(orderBy).limit(limit);
                    } else {
                        y = selectCondition.limit(limit);
                    }
                }
    ...
    public static boolean deleteRow(TableImpl<?> table, Condition condition) {
        Connection conn = null;
        try {
            conn = databaseConnectionPool.getConnection();
            DSLContext create = DSL.using(conn, SQLDialect.POSTGRES);
            create.delete(table).where(condition).execute();
