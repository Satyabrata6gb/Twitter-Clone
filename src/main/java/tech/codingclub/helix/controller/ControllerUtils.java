...
public class ControllerUtils {
    ...
    public static Member getCurrentMember(HttpServletRequest request) {
        Member member = new GenericDB<Member>().getRow(tech.codingclub.helix.tables.Member.MEMBER,
                Member.class, tech.codingclub.helix.tables.Member.MEMBER.ID.eq(
                        ControllerUtils.getUserId(request)));
        if (member != null) {
            ...
            if (cookies != null) {
                for (Cookie cookie : cookies) {
                    if (cookie.getName().equals(TOKEN_COOKIE)) {
                        Member member = new GenericDB<Member>().getRow(Tables.MEMBER, Member.class,
                                tech.codingclub.helix.tables.Member.MEMBER.TOKEN.eq(
                                        cookie.getValue()));
                        if (member != null && member.token !=
                                null) { //.&& author.token_expire_time != null && author
                            // .token_expire_time > new Date
                            // ().getTime()) {
    ...
    public static Boolean isUserAdmin(HttpServletRequest httpServletRequest) {
        ...
        if (cookies != null) {
            for (Cookie cookie : cookies) {
                if (cookie.getName().equals(TOKEN_COOKIE)) {
                    Member member = new GenericDB<Member>().getRow(Tables.MEMBER, Member.class,
                            tech.codingclub.helix.tables.Member.MEMBER.TOKEN.eq(cookie.getValue())
                                    .and(tech.codingclub.helix.tables.Member.MEMBER.ROLE.eq(
                                            "cc-admin")));
                    if (member != null && member.token !=
                            null) { //.&& author.token_expire_time != null && author
                        // .token_expire_time > new Date()
        ...
        if (cookies != null) {
            for (Cookie cookie : cookies) {
                if (cookie.getName().equals(TOKEN_COOKIE)) {
                    Member member = new GenericDB<Member>().getRow(Tables.MEMBER, Member.class,
                            tech.codingclub.helix.tables.Member.MEMBER.TOKEN.eq(cookie.getValue())
                                    .and(tech.codingclub.helix.tables.Member.MEMBER.ROLE.eq("cm")));
                    if (member != null && member.token !=
                            null) { //.&& author.token_expire_time != null && author
                        // .token_expire_time > new Date()
                        // .getTime()) {
